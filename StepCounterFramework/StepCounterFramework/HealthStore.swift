...
public class HealthStore {
    ...
    public func requestAuthorisation(completion: @escaping(Bool)-> Void) {
        ...
        if #available(iOS 12.0, *) {
            ...
            healthStore.getRequestStatusForAuthorization(toShare: [stepCount], read: [stepCount]) { (requestAuthorisationStatus, err) in
                
                if (requestAuthorisationStatus == .shouldRequest) || (requestAuthorisationStatus == .unnecessary) {
                    healthStore.requestAuthorization(toShare: [stepCount], read: [stepCount]) { (isAuthorised, error) in
    ...
    public func calculateSteps(completion: @escaping (HKStatisticsCollection?, Double?)->Void) {
        ...
        let startDate = Date().adjust(for: .startOfDay)
    ...
    public func getSteps(for selectedDate: Date, completion: @escaping (HKStatisticsCollection?, Double?)->Void) {
        var stepsCount = 0.0
        let stepType = HKQuantityType.quantityType(forIdentifier: HKQuantityTypeIdentifier.stepCount)!

        let startDate = selectedDate.adjust(for: .startOfDay)
        
        let anchorDate = Date.mondayAt12AM()
        let daily = DateComponents(day: 1)

        let predicate = HKQuery.predicateForSamples(withStart: startDate, end: selectedDate.adjust(for: .endOfDay), options: .strictStartDate)
        ...
        _query?.initialResultsHandler = {
            ...
            statisticsCollection?.enumerateStatistics(from: startDate ?? Date(), to: selectedDate.adjust(for: .endOfDay) ?? Date()) { statstics, stop in
